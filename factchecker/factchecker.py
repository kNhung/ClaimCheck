...
class FactChecker:
        ...
        try:
            ...
            if m:
                ...
                if action.lower() == 'web':
                    ...
                    # Invalidate cache since report changed
                    self.invalidate_report_cache()
                    # Don't save JSON after every action, only after batch
    ...
    def run(self):
        ...
        # For planning, use shorter record to save tokens (planning doesn't need full history)
        planning_record = self.get_report(max_length=4000)
        actions = planning.plan(self.claim, record=planning_record, actions=actions)
